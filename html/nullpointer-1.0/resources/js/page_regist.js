	...
	var validate = $("#signupForm").validate({
		...
		messages: {
			email: {
				...
				email: $.i18n.prop("请输入合法邮箱")
			},
			password: {
				required: $.i18n.prop("请输入密码"),
				minlength: jQuery.format($.i18n.prop("输入合法密码")),
				maxlength: jQuery.format($.i18n.prop("密码的最大长度"))
			},
			passwordAgain: {
				required: $.i18n.prop("请再次输入密码"),
				minlength: jQuery.format($.i18n.prop("输入合法密码")),
				maxlength: jQuery.format($.i18n.prop("密码的最大长度")),
				equalTo: jQuery.format($.i18n.prop("两次输入密码不同"))
			},
	
			qq: {
				required: $.i18n.prop("请输入qq"),
				digits: $.i18n.prop("输入正确的qq")
			}
		}
	});
