...
public class HibernateSearchDaoImpl extends BaseDao<Bug, String> {
	...
	public Page<Bug> findBugByPage(int pageNum, int pageSize, String search) throws Exception {
		...
		try {
			...
			// 集关键字高亮的实现代码
			// SimpleHTMLFormatter formatter = new SimpleHTMLFormatter("<font
			// color='red';>", "</font>");
			...
			// .invokeMethod(BeanUtils.getPropertyDescriptor(Bug.class,
			// fieldName).getReadMethod(), q);
			// String hightLightFieldValue = null;
			// if (fieldValue instanceof String) {
			// // 获得高亮关键字
			// System.out.println("bug 高亮" + fieldValue);
			// hightLightFieldValue = highlighter.getBestFragment(analyzer,
			// fieldName,
			...
			// BeanUtils.getPropertyDescriptor(Bug.class,
			// fieldName).getWriteMethod(), q,
	...
	public Page<Question> findQuestionByPage(int pageNum, int pageSize, String search) throws Exception {
		...
		try {
			// 模糊查询
			org.apache.lucene.search.Query luceneQuery = qb.keyword().onFields("questionTitle", "questionDescribe")
					.matching(search).createQuery();
			// org.apache.lucene.search.Query luceneQuery =
			// qb.keyword().fuzzy().withThreshold(1f)
			// .onFields("questionTitle",
			// "questionDescribe").matching(search).createQuery();
			Query hibQuery = fullTextSession.createFullTextQuery(luceneQuery);
			hibQuery.setFirstResult((pageNum - 1) * pageSize);
			hibQuery.setMaxResults(pageSize);
			list = hibQuery.list();
			...
			// BeanUtils.getPropertyDescriptor(Question.class,
			// fieldName).getReadMethod(), q);
			// String hightLightFieldValue = null;
			// if (fieldValue instanceof String) {
			// // 获得高亮关键字
